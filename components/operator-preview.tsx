...
export type PreviewRow = {
  ldap: string; operator: string; periodLabel: string; scoreNumber: number | null;
...
export function OperatorPreview({ profile, rows, links, title = 'Мої KPI' }: { profile: PreviewProfile; rows: PreviewRow[]; links: HelpfulLink[]; title?: string }) {
  const latest = rows[0];
  const metrics = [
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><div className="border-b border-slate-100 pb-5"><p className="text-sm font-bold uppercase tracking-[0.14em] text-blue-700">{title}</p><h2 className="mt-2 text-3xl font-black tracking-tight">{profile.operator || profile.ldap}</h2><p className="mt-2 text-sm text-slate-500">{latest?.periodLabel || 'Період ще не внесено'} · {profile.ldap} · {profile.direction || '—'} · {profile.level || '—'} {profile.independence ? `· Самостійність: ${profile.independence}` : ''}</p></div>
